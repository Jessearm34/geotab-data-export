...
from models import Driver, FaultCode, GPSLog, Trip, Vehicle
...
class AnalyticsService:
    ...
    # Geotab Trip entity has no fuelUsed property — fuel/MPG metrics unavailable.
    ...
    def maintenance_metrics(self, since: datetime | None = None, until: datetime | None = None) -> dict[str, Any]:
        since, until = self._since(since), self._until(until)
        fault_rows = list(self.db.execute(
    ...
    def fuel_efficiency(self, since: datetime | None = None, until: datetime | None = None) -> list[dict[str, Any]]:
        ...
        for r in rows:
            miles = float(r["miles"])
            mpg = None  # no fuel data available
            result.append({
                "vehicle_id": r["id"],
